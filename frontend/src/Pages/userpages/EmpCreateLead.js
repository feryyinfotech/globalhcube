...
const EmpCreateLead = () => {
  ...
  const { isLoading, mutate } = useMutation(
    async (values) => {
      ...
      return axiosInstance.post(API_URLS.create_lead_by_emp, reqBody);
    },
    {
      onSuccess: (response) => {
  ...
  const { data } = useQuery(
    ['creation_source'],
    () => axiosInstance.get(API_URLS?.lead_src_list_lead_creation_emp),
    {
  ...
  const { data: project } = useQuery(
    ['project_list'],
    () => axiosInstance.get(API_URLS?.project_list_dropdown_emp),
    {
      refetchOnMount: false,
      refetchOnReconnect: false,
      refetchOnWindowFocus: false,
    ...
    <div className="p-3 md:p-6">
      ...
      <div className="breadcruumb_section">
        <div className="breadcrumb_content">
          ...
          <p>Enter what you know now — only the name and mobile number are required. You can fill the rest after the first call.</p>
        </div>
      </div>
