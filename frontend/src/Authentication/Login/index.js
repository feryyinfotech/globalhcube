  ...
  const loginFunction = async (reqBody) => {
    ...
    try {
      ...
      if (response?.data?.msg === 'Login Successfully') {
        console.log(response?.data?.response?.[0]?.token);
        localStorage.setItem('token', response?.data?.response?.[0]?.token);
        localStorage.setItem('name', response?.data?.response?.[0]?.email || '');
        localStorage.setItem(
          'user_type',
          response?.data?.response?.[0]?.user_type || 'Admin'
        );
        navigate('/dashboard');
          ...
          <Row>
            ...
            <Col xl={6} lg={6} md={6} sm={12} xs={12} >
              <div className="login_right_side">
                ...
                <form onSubmit={fk.handleSubmit} className="space-y-5 sm:space-y-6">
                  <div className='field'>
                    ...
                    <div className="input-groups">
                      ...
                      <div className="p-2 text-white/80 shrink-0">
                  ...
                  <div className='field'>
                    ...
                    <div className="input-groups">
                      ...
                      <div className="p-2 text-white/80 shrink-0">
                        <AiFillLock className="w-5 h-5 sm:w-6 sm:h-6" />
                      </div>
                    </div>
                  </div>
